 ...
 * bundle:
 *
 * - DSH ≥ 0.1.7 (modern): the official Plugins page, bound to the
 *   profile-entry settings namespace through `configForms`
 *   (`ctx.configForms.get(entryId)`). The form registers into
 *   `plugins.bundle.config` keyed by the bundle's package name, which the
 *   bundle's page renders under its description — the page a card in the
 *   Plugins list opens, so the settings are one click from that list (the
 *   shape the official plugins use). A host that declares only the row slot —
 *   no released host does (see AGENTS.md) — gets the row's configure control
 *   instead; the two are mutually exclusive, never both.
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * `plugins.bundle.config` key: the bundle's own package name. The bundle
     * page renders this entry between its description and its rows, and that
     * page is exactly what a card in the Plugins list opens — so this is the
     * slot that keeps the settings one click from the list. Must match the
     * installed package name in `package.json`.
     */
    var BUNDLE_KEY = "dsh-web-icon-indicator";
    ...
    function apply(ctx) {
      ...
      // One settings card, three page slots across the host generations.
      // `ctx.slots.inject(key, …)` is a no-op for a slot this host never
      // declares, so `start` runs exactly the branches the running host
      // understands. A DSH ≥ 0.1.7 host declares BOTH modern slots, so those
      // two branches are mutually exclusive:
      //   - `plugins.bundle.config` (≥ 0.1.7): keyed by the bundle's own package
      //     name. The bundle page renders it under the description, and that
      //     page is exactly what a card in the Plugins list opens — so the
      //     settings sit one click from the list. This is the shape the
      //     official plugins use (the official voice-input bundle registers
      //     this slot).
      //   - `plugins.row.config` (≥ 0.1.6-alpha.2): keyed `<package>#<row id>`,
      //     it only adds the configure control on the bundle page's row, which
      //     buries the form one level deeper (bundle page -> row -> configure).
      //     Registered only where the bundle slot is absent — a guard no
      //     released host reaches, since every host with a Plugins page
      //     declares the bundle slot too (see AGENTS.md).
      //   - `settings.plugin.item` (≤ 0.1.6-alpha.1): keyed by the legacy
      //     settings namespace, dispatched by the settings tab per served
      //     namespace.
      var start = function () {
        var rowInject = null;
        var bundleLive = false;
        var dropRow = function () {
          var cancelRow = rowInject;
          rowInject = null;
          if (cancelRow !== null) cancelRow();
        };
        var disposeBundle = ctx.slots.inject("plugins.bundle.config", function () {
          bundleLive = true;
          // The bundle slot owns the card wherever it exists: cancel the row
          // wait, or dispose a row registration the host declared first, so the
          // same form is never drawn twice on one page.
          dropRow();
        ...
        if (!bundleLive) {
          rowInject = ctx.slots.inject("plugins.row.config", function () {
            // The bundle slot may still be declared after this one (both slots
            // arrive with the Plugins page's own registration): stay out of its
            // way rather than registering a second copy of the same card.
            if (bundleLive) return function () {};
            return ctx.slots.register(
              {
        ...
        return function () { disposeBundle(); dropRow(); disposeItem(); };
      };
