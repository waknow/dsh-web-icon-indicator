...
(function () {
  try {
    ...
    // Every DSH shell up to 0.1.6-alpha.2 carried ONE <link rel=icon>. From
    // 0.1.7-alpha.1 the shell ships a THEME-SCOPED PAIR instead:
    //   <link rel=icon href=favicon-dark.svg media="(prefers-color-scheme: dark)">
    //   <link rel=icon href=favicon.svg      media="(prefers-color-scheme: light)">
    // Both links carry rel=icon, so querying just the first one and repainting
    // it was silently useless: the browser resolves the favicon to the LAST
    // connected link whose media condition matches, i.e. the second one — the
    // one this script never touched. The tab then kept the shell's icon forever
    // while the status endpoint changed underneath (the "icon never changes"
    // bug). Normalize to ONE always-matching link first, then keep painting it.
    function normalizeIconLinks() {
      try {
        var all = document.querySelectorAll("[rel~='icon']");
        if (!all || !all.length) return;
        var pick = all[0];
        for (var i = 1; i < all.length; i++) {
          // Prefer the variant the current scheme actually selects, so a
          // dark-mode tab keeps the shell's dark artwork as its resting state.
          var m = all[i].getAttribute("media");
          if (m && window.matchMedia && window.matchMedia(m).matches) pick = all[i];
        }
        for (var j = 0; j < all.length; j++) {
          var node = all[j];
          if (node !== pick && typeof node.remove === "function") { try { node.remove(); } catch (e) {} }
        }
        // Drop the media condition: a link that no longer matches the scheme
        // would be dead weight, and the frame we paint is scheme-agnostic.
        try { pick.removeAttribute("media"); } catch (e) {}
      } catch (e) {}
    }
