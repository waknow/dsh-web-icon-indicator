...
*     permission decision. This covers sandbox interceptions: a denied
*     sandbox call that escalates via `sandbox_permissions` +
*     `justification` routes through the approval service, which appends
*     an `approval/asked` event and blocks the agent until the user
*     approves or rejects.
...
export default {
  ...
  apply(ctx) {
    ...
    // into `asking` exactly like ask_user_question. The approval service appends
    // `approval/asked` before asking and `approval/decided` after the decision,
    // so the session event feed is the single observable signal — and it covers
    // sandbox escalations (sandbox_permissions + justification route through it).
    ...
    // for an `approval/asked` that no `approval/decided` has closed. This is a
    // safety net on top of the reactive `session/event` feed — it also catches
    // approvals that were appended while this plugin was unmounted or an event
    // was missed, matching the reconcile() design of deriving from the durable
    // log rather than trusting delivery.
