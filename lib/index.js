...
export default {
  ...
  apply(ctx) {
    const user = ctx.get("config") || {};
    // Merge each configured state over its default: { effect, colors, speed }.
    const stateConfigs = {};
    for (const name of STATE_NAMES) {
      const merged = { ...(DEFAULTS.states[name] || {}), ...(user.states?.[name] || {}) };
    ...
    const cfg = { ...DEFAULTS, ...user, states: stateConfigs };
    ...
    const script = INJECTED_SCRIPT
      ...
      .replace("__CFG__", JSON.stringify({ states: cfg.states }));
