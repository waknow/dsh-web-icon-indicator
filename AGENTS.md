...
- The browser script is the `INJECTED_SCRIPT` template string, injected via `webServer.tapIndex`; config flows in through placeholder tokens (`__STATUS_PATH__`, `__BASE_PATH__`, `__CFG__`), each paired with a `.replace()` call in `apply()`. `__CFG__` carries the `{ colors, effects, blinkColor, askingBlinkMs, effectSpeedMs }` object as JSON.
...
1. Icon geometry: edit `icons/base.svg` (keep the `__COLOR__` placeholder in `#p { fill: … }`). Colors/effects/timings: edit `DEFAULTS` in `lib/index.js` (`colors` / `effects` / `blinkColor` / `askingHoldMs` / `askingBlinkMs` / `doneHoldMs` / `effectSpeedMs`).
...
1. Add a color/effect default for the new state in `DEFAULTS` (`colors` / `effects`) in `lib/index.js`. No new SVG is needed — every state renders from `base.svg`.
