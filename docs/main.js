...
(function () {
  ...
  var I18N = {
    zh: {
      ...
      "config.p1": "DSH ≥ 0.1.2 时，Web GUI 的「设置 → 插件 → 插件配置」里会出现一张 Favicon indicator 卡片：顶部是**默认图标颜色**（待机鲸鱼唯一的设置项）——改成一个与众不同的颜色，多开实例时标签页就一眼可分，配得太接近其它状态色时卡片会当场告警；下面是「配色一览」，以及提问/完成驻留和 running / asking / done 三个状态的详细配置。颜色一律以**色块**呈现，点击即打开系统取色器，卡片本身不打印十六进制文本。",
    ...
    en: {
      ...
      "config.p1": "On DSH ≥ 0.1.2, a Favicon indicator card appears under Settings → Plugins → Plugin config. It leads with the **default icon color** — the idle whale's only knob; give it a value of its own and multiple instances become distinguishable at a glance, and the card warns on the spot when it sits too close to another state's color. Below it: a palette preview, the two hold timings, and the per-state effect / colors / cycle for running, asking and done. Colors are shown as chips you click to open the native picker — the card itself prints no hex text.",
