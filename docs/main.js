...
(function () {
  ...
  var I18N = {
    zh: {
      ...
      "config.p1": "DSH ≥ 0.1.2 时，Web GUI 的「设置 → 插件」里会出现一张 Favicon indicator 卡片，点开即见完整设置：顶部是**默认图标颜色**（待机鲸鱼唯一的设置项）——改成一个与众不同的颜色，多开实例时标签页就一眼可分，配得太接近其它状态色时卡片会当场告警；下面是「配色一览」，以及提问/完成驻留和 running / asking / done 三个状态的详细配置。颜色一律以**色块**呈现，点击即打开系统取色器，卡片本身不打印十六进制文本。",
      "config.p2": "改动保存进 profile 的 settings.yaml，并在约 1 秒内同步到正在运行的标签页——无需重启宿主，也无需刷新页面。",
      "config.step1": "打开 DSH Web GUI，进入「设置」",
      "config.step2": "切换到「插件」标签页，点开 Favicon indicator 卡片",
      "config.step3": "展开「Favicon indicator / 标签页图标指示器」卡片：先设默认图标颜色，再逐状态编辑",
      "config.mock.title": "Favicon indicator", "config.mock.sub": "插件配置",
      "config.mock.askhold": "提问驻留", "config.mock.donehold": "完成驻留",
      "config.mock.default": "默认图标颜色", "config.mock.defaultv": "待机 · 一种颜色",
      "config.mock.staticv": "静态", "config.mock.blinkv": "闪烁 · 400ms",
      ...
      "install.note": "需要 DSH ≥ 0.1.2（设置界面所需）。安装后打开 Web GUI → 设置 → 插件，点开 Favicon indicator 卡片即可配置。",
      "copy": "复制", "copied": "已复制 ✓",
    ...
    en: {
      ...
      "config.p1": "On DSH ≥ 0.1.2, a Favicon indicator card appears under Settings → Plugins — open it and the full form is right there. It leads with the **default icon color** — the idle whale's only knob; give it a value of its own and multiple instances become distinguishable at a glance, and the card warns on the spot when it sits too close to another state's color. Below it: a palette preview, the two hold timings, and the per-state effect / colors / cycle for running, asking and done. Colors are shown as chips you click to open the native picker — the card itself prints no hex text.",
      ...
      "config.step2": "Open the Plugins tab, then the Favicon indicator card",
      "config.step3": "Expand the Favicon indicator card: set the default color first, then edit the states",
      ...
      "config.mock.default": "Default icon color", "config.mock.defaultv": "idle · one color",
      "config.mock.staticv": "Static", "config.mock.blinkv": "Blink · 400ms",
      ...
      "install.note": "Requires DSH ≥ 0.1.2 (for the settings UI). After installing, open the Web GUI → Settings → Plugins and open the Favicon indicator card to configure it.",
